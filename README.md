...
+ ## 모든것이 HTTP
  ...
  + ### HTTP 헤더
    ...
    + ### 특별한 정보
      + #### Host
        + ![http-special-info-host.png](images/http-special-info-host.png)
        + 정의 : 요청한 호스트 정보 (도메인)
          + 필수
          + 하나의 서버가 여러 도메인을 처리해야 할 때
          + 하나의 주소에 여러 도메인이 적용되어 있을 때
          + 요청에서 사용
      + #### Location
        + 정의 : 페이지 Redirection
          ...
          + 3xx (Redirection) : Location 값은 요청을 자동으로 Redirection하기 위한 대상 리소스를 가리킴
      + #### Allow
        + 정의 : 허용 가능한 HTTP 메서드
          ...
          + 예) 
            + Allow : GET, HEAD, PUT
      + #### Retry-After
        + 정의 : 유저 에이전트가 다음 요청을 하기 까지 기다려야 하는 시간
          + 503 (Service Unavailable) : 서비스가 언제까지 불능인지 알려줄 수 있음
          + Retry-After (날짜 표기) : Fir, 31 Dec 1999 23:59:59 GMT
          + Retry-After (초 단위 표기) : 120
    
    + ### 인증
      + #### Authorization
        + 정의 : 클라이언트 인증 정보를 서버에 전달
          + 예) Authorization : Basic xxxxxxxxxxxxxxxx
      + #### WWW-Authenticate
        + 정의 : 리소스 접근시 필요한 인증 방법 정의
    ...
    + ### 쿠키
      + ### 정의
        ...
        + 로그인 이후 페이지 접근
          + ![after-cookie-1.png](images%2Fafter-cookie-1.png)
          + ![after-cookie-2.png](images/after-cookie-2.png)
        ...
        + <span style="color:red"><U>**주의!**</U></span>
          + 보안에 민감한 데이터는 저장하면 안됨(주민번호, 신용카드 번호 등등)
    + ### 생명주기 (expires)
      + Set-Cookie: expires=Sat, 26-Dec-2020 00:00:00 GMT 04:39:21 GMT
        + 만료일이 되면 쿠키 삭제
